...
export default function Home(){
  ...
  async function savePointGroup(groupItem:ReturnType<typeof group>[number],patch:{location:string;district:string;longitude?:number;latitude?:number},themeNames:string[],stations:Station[]){if(!(await ensureAdmin()))return false;const validStationIds=new Set(stations.map(station=>station.id));setPoints(items=>items.map(point=>point.id===groupItem.point.id?{...point,...patch,themeNames,stations,coordinateSystem:patch.longitude&&patch.latitude?"gcj02":point.coordinateSystem}:point));setTasks(items=>items.map(task=>task.pointId===groupItem.point.id?{...task,stationIds:(task.stationIds||[]).filter(id=>validStationIds.has(id))}:task));setPointEditOnOpen(false);setExpanded(null);return true}
...
<span className="chevron">↗</span>
  ...
  {view==="library"&&activeGroup&&<PointDetailModal key={`${activeGroup.key}-${pointEditOnOpen?"edit":"view"}`} point={activeGroup} initialEditing={pointEditOnOpen} districts={availableDistricts} categories={themeCategories} onClose={()=>{setExpanded(null);setPointEditOnOpen(false)}} onSave={(patch,names,stations)=>savePointGroup(activeGroup,patch,names,stations)} onAddTask={()=>addTask(activeGroup)} onManageTask={async id=>{if(await openEditor(id))setExpanded(null)}} onRemoveTask={removeTask} onChangeStatus={changeStatus}/>}
