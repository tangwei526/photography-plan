...
export default function Home(){
  ...
  async function savePointGroup(point:ReturnType<typeof group>[number],patch:{location:string;district:string;longitude?:number;latitude?:number},themeNames:string[]){if(!themeNames.length){alert("请至少选择一个拍摄主题");return false}if(!(await ensureAdmin()))return false;setTasks(items=>{const groupItems=items.filter(task=>`${task.district}::${task.location}`===point.key);const remaining=items.filter(task=>`${task.district}::${task.location}`!==point.key);let nextId=Math.max(0,...items.map(task=>task.id))+1;const source=groupItems[0]||point.tasks[0];const selected=themeNames.map(name=>{const existing=groupItems.find(task=>(task.themeCategory||task.theme)===name);if(existing)return {...existing,...patch,themeCategory:name,coordinateSystem:patch.longitude&&patch.latitude?"gcj02":existing.coordinateSystem};return {id:nextId++,...patch,priority:"低" as Priority,theme:name,themeCategory:name,methods:["待规划"],media:["照片"],clarity:"中",status:"未拍摄" as Status,note:"",sourceRow:0,coordinateSystem:patch.longitude&&patch.latitude?"gcj02":source.coordinateSystem,stations:[],samples:[]}});return [...remaining,...selected]});setPointEditOnOpen(false);setExpanded(null);return true}
  ...
  async function removePointTheme(point:ReturnType<typeof group>[number],task:Task){if(point.tasks.length===1){alert("这是该点位的最后一个主题。请先关联一个新主题，再移除当前主题；点位不会被删除。");return}if(!confirm(`从该点位移除“${task.theme}”主题？`)||!(await ensureAdmin()))return;setTasks(items=>items.filter(item=>item.id!==task.id))}
...
<span className="chevron">↗</span>
  ...
  {view==="library"&&activeGroup&&<PointDetailModal key={`${activeGroup.key}-${pointEditOnOpen?"edit":"view"}`} point={activeGroup} initialEditing={pointEditOnOpen} districts={availableDistricts} categories={themeCategories} onClose={()=>{setExpanded(null);setPointEditOnOpen(false)}} onSave={(patch,names)=>savePointGroup(activeGroup,patch,names)} onManageTheme={async id=>{if(await openEditor(id))setExpanded(null)}} onRemoveTheme={task=>removePointTheme(activeGroup,task)} onChangeStatus={changeStatus}/>}
...
function PointDetailModal({point,initialEditing,districts,categories,onClose,onSave,onManageTheme,onRemoveTheme,onChangeStatus}:{point:ReturnType<typeof group>[number];initialEditing?:boolean;districts:string[];categories:string[];onClose:()=>void;onSave:(patch:{location:string;district:string;longitude?:number;latitude?:number},themeNames:string[])=>Promise<boolean>;onManageTheme:(id:number)=>void;onRemoveTheme:(task:Task)=>void;onChangeStatus:(task:Task)=>void}){
  const source=point.tasks[0];const linkedThemes=point.tasks.map(task=>task.themeCategory||task.theme).filter(Boolean);const themeOptions=[...new Set([...categories,...linkedThemes])];
  const [editingPoint,setEditingPoint]=useState(Boolean(initialEditing));const [saving,setSaving]=useState(false);const [formError,setFormError]=useState("");const [selectedThemes,setSelectedThemes]=useState<string[]>(linkedThemes.filter(name=>categories.includes(name)));const [draft,setDraft]=useState({location:point.location,district:point.district,longitude:source.longitude?String(source.longitude):"",latitude:source.latitude?String(source.latitude):""});
  useEffect(()=>{const previous=document.body.style.overflow;document.body.style.overflow="hidden";return()=>{document.body.style.overflow=previous}},[]);
  async function save(){const longitude=n(draft.longitude);const latitude=n(draft.latitude);if(!draft.location.trim()){setFormError("请填写点位名称");return}if(!longitude||!latitude){setFormError("请在地图中搜索并选择拍摄位置");return}if(!selectedThemes.length){setFormError("请至少选择一个拍摄主题");return}setSaving(true);setFormError("");try{if(await onSave({location:draft.location.trim(),district:draft.district,longitude,latitude},selectedThemes))onClose()}finally{setSaving(false)}}
...
<div className="modalHead"><div><small>{editingPoint?"EDIT LOCATION":"LOCATION DETAIL"}</small><h2 id="point-dialog-title">{editingPoint?"新建或编辑拍摄点位":point.location}</h2><p>{editingPoint?"搜索地图确定位置，并选择需要关联的创作主题。":`${point.district} · ${point.tasks.length} 个关联主题`}</p></div>{!editingPoint&&<button onClick={onClose} aria-label="关闭点位详情">×</button>}</div>
...
</div>:<><div className="pointOverview"><div><small>拍摄进度</small><strong>{point.tasks.filter(task=>task.status==="已毕业").length} / {point.tasks.length}</strong></div><div><small>点位优先级</small><strong>{point.priority}</strong></div><div><small>地图位置</small><strong>{source.longitude&&source.latitude?"已定位":"待定位"}</strong></div><button onClick={()=>setEditingPoint(true)}>编辑点位信息</button></div>
<div className="pointThemeHead"><div><h3>关联拍摄主题</h3><p>编辑点位时可直接勾选全部创作主题。</p></div></div><div className="pointThemeList">{point.tasks.map(task=><article key={task.id}>
...
<div className="themeGrid">{records.map(record=>{const items=tasks.filter(task=>matchesTheme(task,record.name));const {pointCount}=themeCounts(items);const icon=themeIcon(record.name);return <article className="themeCard" key={record.id}>
...
<PointMapPicker longitude={task.longitude} latitude={task.latitude} district={task.district} onPick={value=>update({longitude:value.longitude,latitude:value.latitude,district:value.district&&districts.includes(value.district)?value.district:task.district,coordinateSystem:"gcj02"})}/>
