...
<PointMapPicker longitude={n(draft.longitude)} latitude={n(draft.latitude)} district={draft.district} onPick={value=>setDraft(current=>({...current,longitude:String(value.longitude),latitude:String(value.latitude),district:value.district&&districts.includes(value.district)?value.district:current.district}))}/>
...
<strong>选择要关联的主题</strong>{available.length?<div className="themePickerOptions">{available.map(name=><label key={name}><input type="checkbox" checked={selectedThemes.includes(name)} onChange={event=>setSelectedThemes(items=>event.target.checked?[...items,name]:items.filter(item=>item!==name))}/><span>{name}</span></label>)}</div>:<p>全部主题均已关联</p>}
<div className="themePickerActions"><button onClick={()=>{setThemePickerOpen(false);setSelectedThemes([])}}>取消</button><button className="primary" disabled={!selectedThemes.length||linking} onClick={linkThemes}>{linking?"正在关联…":`确认关联${selectedThemes.length?`（${selectedThemes.length}）`:""}`}</button></div>
</div>}</div></div>
