...
import { DEFAULT_GEOGRAPHY, cityGeography, cityKey, deriveCityRecords, geographyLabel, mergeCityRecords, normalizeCityRecord, normalizeGeography, pointIdentity, type CityRecord, type Geography } from "@/lib/geography";
...
type View = "library" | "gallery" | "footprint" | "map" | "calendar" | "themes" | "coverage";
...
type PointRecord = Geography & { id:string; location:string; priority:Priority; longitude?:number; latitude?:number; coordinateSystem?:"wgs84"|"gcj02"; themeNames:string[]; stations:Station[] };
type Sample = { id: string; name: string; url: string };
type GallerySample = Partial<Geography>&{ id:string; url:string; uploadedAt:string; size?:number; taskId:string; district:string; location:string; theme:string; themeCategory?:string; device?:string; shootTime?:string; stationId:string; stationName:string; stationDescription:string; subjectDescription?:string; note:string; originalName:string; groupId?:string; local?:boolean };
...
type UploadJob = Partial<Geography>&{ jobId:string; file:File; taskId:string; district:string; location:string; theme:string; themeCategory:string; device:string; shootTime:string; stationId:string; stationName:string; stationDescription:string; note:string; originalName:string; uploadId?:string; objectId?:string; parts:UploadPart[]; status:"waiting"|"uploading"|"failed"; error?:string; createdAt:number };
type Task = Partial<Geography> & {
...
type WeatherLocation = { id?:string; name:string; adm2?:string; adm1?:string; country?:string; countryCode?:string; timezone?:string; latitude:number; longitude:number };
type RouteInfo = { distance:number; duration:number; geometry:[number,number][] };
type AstronomyData = { date:string; sunrise:string; sunset:string; dawn:string; dusk:string; moonrise:string; moonset:string; moonPhase:number; moonIllumination:number; timezone?:string; source?:string };
...
const normalizeGallerySample=(sample:GallerySample):GallerySample=>({...sample,...normalizeGeography(sample),themeCategory:sample.themeCategory||inferThemeCategory(sample.theme),url:sample.url.startsWith("/")?`${sampleApi}${sample.url.slice("/api/samples".length)}`:sample.url});
...
const normalizeTask=(t:Task):Task=>({...t,...normalizeGeography(t),themeCategory:t.themeCategory==="地铁"?"轨道交通":t.themeCategory||inferThemeCategory(t.theme),timeWindow:t.timeWindow||inferTimeWindow(t.theme),stations:t.stations||[],stationIds:t.stationIds||t.stations?.map(station=>station.id)||[],samples:t.samples||[],graduationCriteria:t.graduationCriteria||""});
const normalizeThemeName=(value:string)=>String(value||"").trim().replace(/^地铁$/,"轨道交通").replace(/^大桥$/,"桥梁").replace(/^立交桥$/,"立交").replace(/^太阳月亮同框$/,"日月对齐");
const locationRecordKey=(source:Partial<Geography>&{district:string;location:string})=>`${cityKey(source)}::${source.district}::${source.location}`;
const normalizePoint=(point:Partial<PointRecord>&Pick<PointRecord,"id"|"district"|"location"|"priority">):PointRecord=>({...point,...normalizeGeography(point),themeNames:point.themeNames||[],stations:point.stations||[]} as PointRecord);
const migrateWorkspace=(source:Task[])=>{const normalized=source.map(normalizeTask);const keys=[...new Set(normalized.map(locationRecordKey))];const points:PointRecord[]=keys.map(key=>{const items=normalized.filter(task=>locationRecordKey(task)===key);const first=items[0];const geography=normalizeGeography(first);const stations=[...new Map(items.flatMap(task=>task.stations||[]).map(station=>[station.id||station.name,station])).values()];const themeNames=[...new Set(items.map(task=>normalizeThemeName(task.themeCategory||inferThemeCategory(task.theme))).filter(Boolean))];return{...geography,id:first.pointId||pointIdentity(geography,first.location),location:first.location,priority:[...items].sort((a,b)=>priorityRank[b.priority]-priorityRank[a.priority])[0].priority,longitude:first.longitude,latitude:first.latitude,coordinateSystem:first.coordinateSystem,themeNames,stations}});const pointByKey=new Map(points.map(point=>[locationRecordKey(point),point]));const tasks=normalized.map(task=>{const point=pointByKey.get(locationRecordKey(task))!;return{...task,...normalizeGeography(point),pointId:point.id,stationIds:task.stationIds?.length?task.stationIds:point.stations.map(station=>station.id)}});return{points,tasks}};
...
type MapSearchResult=Partial<Geography>&{name:string;address:string;longitude:number;latitude:number;coordinateSystem?:"wgs84"|"gcj02"};
type MapPick=Partial<Geography>&{longitude:number;latitude:number;coordinateSystem?:"wgs84"|"gcj02"};
function PointMapPicker({longitude,latitude,geography,onPick}:{longitude?:number;latitude?:number;geography:Geography;onPick:(value:MapPick)=>void}){
  const el=useRef<HTMLDivElement>(null);const map=useRef<any>(null);const marker=useRef<any>(null);const onPickRef=useRef(onPick);const geographyRef=useRef(geography);const [ready,setReady]=useState(false);const [message,setMessage]=useState("搜索地点，或点击地图选择点位");const [query,setQuery]=useState("");const [searching,setSearching]=useState(false);const [results,setResults]=useState<MapSearchResult[]>([]);
  onPickRef.current=onPick;geographyRef.current=geography;
  function placePoint(point:MapPick,zoom=17){const AMap=(window as any).AMap;if(!map.current||!AMap)return;if(marker.current)marker.current.setPosition([point.longitude,point.latitude]);else{marker.current=new AMap.Marker({position:[point.longitude,point.latitude],anchor:"bottom-center"});map.current.add(marker.current)}map.current.setZoomAndCenter(zoom,[point.longitude,point.latitude]);onPickRef.current(point)}
  async function searchPlace(){const keyword=query.trim();if(!keyword||searching)return;setSearching(true);setResults([]);setMessage("正在搜索全球地点…");try{let items:MapSearchResult[]=[];if(geography.countryCode==="CN"){const response=await fetch("/api/amap-locate",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"search",query:keyword,...geography})});const data=await response.json().catch(()=>({}));if(response.ok)items=(data.items||[]) as MapSearchResult[]}if(!items.length){const response=await fetch(`/api/location-search?query=${encodeURIComponent(keyword)}`,{cache:"no-store"});const data=await response.json();if(!response.ok)throw new Error(data.error||"全球地点搜索失败");items=(data.items||[]) as MapSearchResult[]}setResults(items);setMessage(items.length?`找到 ${items.length} 个候选地点，请选择后在地图上微调`:`没有找到“${keyword}”，请改搜城市、区县或附近地标，再点击地图微调`)}catch(reason){setMessage(reason instanceof Error?reason.message:"地点搜索失败，请改搜城市或附近地标")}finally{setSearching(false)}}
  ...
  useEffect(()=>{let active=true;(async()=>{try{const AMap=await loadAMap();if(!active||!el.current)return;const center=longitude&&latitude?[longitude,latitude]:[106.551,29.563];map.current=new AMap.Map(el.current,{viewMode:"2D",zoom:longitude&&latitude?16:3,center});map.current.addControl(new AMap.Scale());map.current.on("click",async(event:any)=>{const coordinateSystem=geographyRef.current.countryCode==="CN"?"gcj02":"wgs84";const point={longitude:Number(event.lnglat.getLng()),latitude:Number(event.lnglat.getLat()),coordinateSystem:coordinateSystem as "gcj02"|"wgs84"};placePoint(point);setResults([]);setMessage("正在识别国家、城市与行政区域…");try{const response=await fetch("/api/amap-locate",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"reverse",...point})});const data=await response.json();if(response.ok){const recognized={...geographyRef.current,...data,...point};onPickRef.current(recognized);setMessage(`${data.formattedAddress||"已关联地图坐标"}；可继续点击微调`)}else{onPickRef.current({...geographyRef.current,...point});setMessage(data.error||"坐标已选择，行政区域沿用当前设置")}}catch{onPickRef.current({...geographyRef.current,...point});setMessage("坐标已选择，行政区域沿用当前设置")}});setReady(true)}catch{if(active)setMessage("地图暂时无法加载，请稍后重试")}})();return()=>{active=false;map.current?.destroy();map.current=null;marker.current=null}},[]);
  ...
  return <div className="pointMapField"><div ref={el} className="pointMapPicker"/><div className="pointMapSearch"><Input value={query} onChange={event=>setQuery(event.target.value)} onKeyDown={event=>{if(event.key==="Enter"){event.preventDefault();searchPlace()}}} placeholder="搜索城市、区县、道路、建筑或附近地标" aria-label="搜索全球地图地点"/><Button type="button" size="sm" disabled={searching||!query.trim()} onClick={searchPlace}>{searching&&<Spinner data-icon="inline-start"/>}{searching?"搜索中":"搜索"}</Button>{results.length>0&&<div className="pointMapResults">{results.map((item,index)=><button type="button" key={`${item.longitude}-${item.latitude}-${index}`} onClick={()=>chooseResult(item)}><strong>{item.name}</strong><small>{[item.countryName,item.admin1,item.city,item.district,item.address].filter(Boolean).join(" · ")}</small></button>)}</div>}</div><div className="pointMapHint"><span>⌖</span><strong>{message}</strong><small>{longitude&&latitude?"已确定地图位置，可继续点击微调":"选择后将自动关联国家、城市、区县、时区与地图位置"}</small></div></div>
}
...
function MapCanvas({tasks,route,onPick}:{tasks:Task[];route:RouteInfo|null;onPick:(t:Task)=>void}){
  ...
  useEffect(()=>{if(!ready||!map.current)return;const AMap=(window as any).AMap;map.current.clearMap();const markers=tasks.map(t=>{const [lat,lng]=t.latitude&&t.longitude?[t.latitude,t.longitude]:coord(t);const color=t.status==="已毕业"?"#3e7b61":t.status==="待补拍"?"#d99434":"#e86632";const marker=new AMap.Marker({position:[lng,lat],anchor:"center",title:`${t.location} · ${t.theme}`,content:`<span class="amapSpot" style="--marker:${color}"></span>`});marker.on("click",()=>onPick(t));map.current.add(marker);return marker});if(route?.geometry.length){const line=new AMap.Polyline({path:route.geometry.map(([lat,lng])=>[lng,lat]),strokeColor:"#e86632",strokeWeight:5,strokeOpacity:.9,showDir:true});map.current.add(line);map.current.setFitView([line],false,[45,45,45,45],13)}else if(!defaultViewSet.current){if(markers.length===1)map.current.setZoomAndCenter(13,markers[0].getPosition());else if(markers.length>1)map.current.setFitView(markers,false,[55,55,55,55],13);else map.current.setZoomAndCenter(3,[106.551,29.563]);defaultViewSet.current=true}},[tasks,route,onPick,ready]);
...
const lunarDate=(date:Date,timeZone="Asia/Shanghai")=>{try{return new Intl.DateTimeFormat("zh-CN-u-ca-chinese",{month:"long",day:"numeric",timeZone}).format(date).replace(/\s/g,"")}catch{return "日期暂不可用"}};
...
function AstronomyHero(){
  const fallback:WeatherLocation={name:"重庆市",adm2:"重庆",country:"中国",countryCode:"CN",timezone:"Asia/Shanghai",latitude:29.563,longitude:106.5516};
  const [now,setNow]=useState(()=>new Date());const [data,setData]=useState<AstronomyData|null>(null);const [failed,setFailed]=useState(false);const [location,setLocation]=useState<WeatherLocation>(fallback);
  const timezone=location.timezone||"UTC";const dateKey=new Intl.DateTimeFormat("en-CA",{timeZone:timezone,year:"numeric",month:"2-digit",day:"2-digit"}).format(now);
  useEffect(()=>{const timer=window.setInterval(()=>setNow(new Date()),1000);return()=>window.clearInterval(timer)},[]);
  useEffect(()=>{try{const saved=JSON.parse(localStorage.getItem("shancheng-weather-location")||"") as WeatherLocation;if(saved?.latitude&&saved?.longitude)setLocation({...saved,timezone:saved.timezone||"Asia/Shanghai"})}catch{}const listener=(event:Event)=>{const next=(event as CustomEvent<WeatherLocation>).detail;if(next?.latitude&&next?.longitude)setLocation(next)};window.addEventListener("shancheng-weather-location-change",listener);return()=>window.removeEventListener("shancheng-weather-location-change",listener)},[]);
  useEffect(()=>{let active=true;setFailed(false);fetch(`${astronomyApi}?date=${dateKey}&lat=${location.latitude}&lon=${location.longitude}&timezone=${encodeURIComponent(timezone)}`).then(response=>{if(!response.ok)throw new Error();return response.json()}).then(value=>{if(active)setData(value)}).catch(()=>{if(active)setFailed(true)});return()=>{active=false}},[dateKey,location.latitude,location.longitude,timezone]);
  ...
  const localClock=new Intl.DateTimeFormat("en-GB",{timeZone:timezone,hour:"2-digit",minute:"2-digit",hour12:false}).format(now);const minutesNow=clockMinutes(localClock)||0;const nextEvent=events.map(event=>({...event,minutes:clockMinutes(event.time)})).filter((event):event is typeof event&{minutes:number}=>event.minutes!==null&&event.minutes>=minutesNow).sort((a,b)=>a.minutes-b.minutes)[0];
  ...
  return <section className="astronomyHero" aria-label="今日天象与拍摄时间窗口">
    <header className="astroHeader"><span className="astroLocation"><LocateFixedIcon aria-hidden="true"/>{location.name} · 今日光线窗口</span><Badge variant="outline" className="astroStatus" aria-live="polite">{statusText}</Badge></header>
    <div className="astroLead"><strong suppressHydrationWarning>{localClock}</strong><p suppressHydrationWarning>{now.toLocaleDateString("zh-CN",{timeZone:timezone,year:"numeric",month:"long",day:"numeric",weekday:"long"})}</p><small suppressHydrationWarning>农历 {lunarDate(now,timezone)} · {timezone}</small></div>
...
function CurrentWeatherCard(){
  ...
  async function useCurrentLocation(){if(!navigator.geolocation){setMessage("当前浏览器不支持定位");return}setLoading(true);setMessage("");navigator.geolocation.getCurrentPosition(async position=>{const coordinates={latitude:position.coords.latitude,longitude:position.coords.longitude};let next:WeatherLocation={...coordinates,name:"当前位置",timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||"UTC"};try{const lookup=await fetch(`${weatherApi}?mode=search&query=${encodeURIComponent(`${coordinates.longitude},${coordinates.latitude}`)}`).then(response=>response.json());if(lookup.locations?.[0])next=lookup.locations[0]}catch{}await load(next)},()=>{setLoading(false);setMessage("无法读取当前位置，可搜索城市切换")},{enableHighAccuracy:false,timeout:8000,maximumAge:600000})}
  ...
  return <section className="currentWeather" aria-label="当前地点实时天气">
    ...
    {searchOpen&&<form className="weatherSearch" onSubmit={search}><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="搜索全球城市或区县" aria-label="搜索全球天气地点"/><button disabled={searching||!query.trim()}>{searching?"搜索中":"搜索"}</button>{results.length>0&&<div className="weatherSearchResults">{results.map(item=><button type="button" key={`${item.id}-${item.latitude}`} onClick={()=>{load(item);setSearchOpen(false);setResults([]);setQuery("")}}><strong>{item.name}</strong><small>{[item.country,item.adm1,item.adm2,item.timezone].filter(Boolean).join(" · ")}</small></button>)}</div>}</form>}
...
function OverviewStats({pointCount,cityCount,counts,scheduleCount,coordinateCount}:{pointCount:number;cityCount:number;counts:{unshot:number;redo:number;done:number};scheduleCount:number;coordinateCount:number}){
  return <section className="stats">
    <article><span className="statIcon orange">⌖</span><div><small>独立点位</small><strong>{pointCount}<i>个</i></strong><em>覆盖 {cityCount} 个城市</em></div></article>
...
export default function Home(){
  ...
  const [cityRecords,setCityRecords]=useState<CityRecord[]>(()=>deriveCityRecords(baseWorkspace.points,baseWorkspace.tasks));
  const [themeMode,setThemeMode]=useState<"light"|"dark">("dark");
  const [country,setCountry]=useState("ALL"); const [city,setCity]=useState("ALL"); const [district,setDistrict]=useState("全部区县"); const [status,setStatus]=useState("全部状态"); const [priority,setPriority]=useState("全部优先级"); const [category,setCategory]=useState("全部归类"); const [query,setQuery]=useState(""); const [pointPage,setPointPage]=useState({key:"",limit:48}); const [routeQuery,setRouteQuery]=useState("");
  ...
  useEffect(()=>{try{const savedWorkspace=localStorage.getItem("shancheng-photo-workspace-v5")||localStorage.getItem("shancheng-photo-workspace-v4")||localStorage.getItem("shancheng-photo-workspace-v3");if(savedWorkspace){const parsed=JSON.parse(savedWorkspace) as {points:PointRecord[];tasks:Task[];cities?:CityRecord[]};if(Array.isArray(parsed.points)&&Array.isArray(parsed.tasks)){const savedPoints=parsed.points.map(normalizePoint);const savedTasks=parsed.tasks.map(normalizeTask);setPoints(savedPoints);setTasks(savedTasks);setCityRecords(Array.isArray(parsed.cities)?mergeCityRecords(parsed.cities.map(normalizeCityRecord),savedPoints,savedTasks):deriveCityRecords(savedPoints,savedTasks))}}else{const legacy=localStorage.getItem("shancheng-photo-tasks-v2")||localStorage.getItem("shancheng-photo-tasks-v1");if(legacy){const migrated=migrateWorkspace(JSON.parse(legacy) as Task[]);setPoints(migrated.points);setTasks(migrated.tasks);setCityRecords(deriveCityRecords(migrated.points,migrated.tasks))}}}catch{}setThemeMode(document.documentElement.dataset.theme==="dark"?"dark":"light");setHydrated(true)},[]);
  useEffect(()=>{if(hydrated)localStorage.setItem("shancheng-photo-workspace-v5",JSON.stringify({version:5,points,tasks,cities:cityRecords}))},[points,tasks,cityRecords,hydrated]);
  useEffect(()=>{if(!hydrated)return;const timer=window.setTimeout(()=>setCityRecords(current=>mergeCityRecords(current,points,tasks)),0);return()=>window.clearTimeout(timer)},[points,tasks,hydrated]);
  ...
  const taskViews=useMemo(()=>tasks.map(task=>{const point=points.find(item=>item.id===task.pointId);return point?{...task,...normalizeGeography(point),location:point.location,longitude:point.longitude,latitude:point.latitude,coordinateSystem:point.coordinateSystem,stations:point.stations}:task}),[tasks,points]);
  const groups=useMemo(()=>group(points,taskViews),[points,taskViews]);
  const countries=useMemo(()=>[...new Map([...cityRecords,...points].map(item=>[item.countryCode,{code:item.countryCode,name:item.countryName}])).values()].sort((a,b)=>a.name.localeCompare(b.name,"zh-CN")),[cityRecords,points]);
  const cities=useMemo(()=>[...new Map([...cityRecords,...points].filter(item=>country==="ALL"||item.countryCode===country).map(item=>[cityKey(item),{key:cityKey(item),name:item.city||item.admin1,countryCode:item.countryCode,admin1:item.admin1}])).values()].sort((a,b)=>a.name.localeCompare(b.name,"zh-CN")),[cityRecords,points,country]);
  const districts=useMemo(()=>[...new Set(points.filter(point=>(country==="ALL"||point.countryCode===country)&&(city==="ALL"||cityKey(point)===city)).map(point=>point.district))].sort((a,b)=>a.localeCompare(b,"zh-CN")),[points,country,city]);
  const activeGroup=groups.find(item=>item.key===expanded);
  ...
  const filtered=useMemo(()=>groups.filter(g=>(country==="ALL"||g.point.countryCode===country)&&(city==="ALL"||cityKey(g.point)===city)&&(district==="全部区县"||g.district===district)&&(status==="全部状态"||g.status===status)&&(priority==="全部优先级"||g.priority===priority)&&(category==="全部归类"||g.themeNames.includes(category))&&`${g.location} ${geographyLabel(g.point,"district")} ${g.themeNames.join(" ")} ${g.tasks.map(t=>`${t.timeWindow||""} ${t.theme} ${t.methods} ${t.note}`).join(" ")}`.toLowerCase().includes(query.toLowerCase())).sort((a,b)=>priorityRank[b.priority]-priorityRank[a.priority]),[groups,country,city,district,status,priority,category,query]);
  const pointFilterKey=`${country}\u0000${city}\u0000${district}\u0000${status}\u0000${priority}\u0000${category}\u0000${query}`;const pointDisplayLimit=pointPage.key===pointFilterKey?pointPage.limit:48;
  ...
  async function importExcel(file:File){
    ...
    const citySheet=wb.Sheets["城市足迹"];
    const importedCities=citySheet?XLSX.utils.sheet_to_json<Record<string,unknown>>(citySheet,{defval:""}).map(row=>normalizeCityRecord({countryCode:String(row["国家代码"]||"").trim(),countryName:String(row["国家"]||"").trim(),admin1:String(row["省/州"]||"").trim(),city:String(row["城市"]||"").trim(),timezone:String(row["时区"]||"UTC").trim(),status:String(row["点亮状态"]||"")==="已点亮"?"已点亮":"计划前往",visitedAt:String(row["到访日期"]||"").trim(),longitude:n(row["城市中心经度"]),latitude:n(row["城市中心纬度"]),note:String(row["城市备注"]||"").trim()})).filter(record=>record.city):[];
    let previous={...DEFAULT_GEOGRAPHY,location:""};
    const creativeThemes=new Map<string,Set<string>>();
    const imported:Task[]=rows.map((r,i)=>{
      const geography=normalizeGeography({
        countryCode:String(r["国家代码"]||previous.countryCode).trim(),countryName:String(r["国家"]||previous.countryName).trim(),
        admin1:String(r["省/州"]||r["省州"]||previous.admin1).trim(),city:String(r["城市"]||previous.city).trim(),
        district:String(r["区县"]||r["行政区域"]||previous.district).trim(),timezone:String(r["时区"]||previous.timezone).trim(),
        formattedAddress:String(r["完整地址"]||"").trim(),
      });
      const locationName=String(r["点位名称"]||previous.location||`未命名点位 ${i+1}`).trim();previous={...geography,location:locationName};
      const station=String(r["机位名称"]||r["关联机位"]||"").trim();const theme=String(r["拍摄任务"]||r["拍摄主题"]||"常规记录");const themeNames=split(r["创作主题"]||r["主题归类"]).map(normalizeThemeName);
      const pointKey=locationRecordKey({...geography,location:locationName});creativeThemes.set(pointKey,new Set([...(creativeThemes.get(pointKey)||[]),...themeNames]));
      const longitude=n(r["经度"]),latitude=n(r["纬度"]);const coordinateSystem:Task["coordinateSystem"]=longitude&&latitude?(String(r["坐标系"]||"").toLowerCase()==="gcj02"?"gcj02":"wgs84"):undefined;
      return {...geography,id:i+1,location:locationName,priority:(["高","中","低"].includes(String(r["点位优先级"]||r["优先级"]))?String(r["点位优先级"]||r["优先级"]):"低") as Priority,theme,timeWindow:String(r["拍摄时间"]||"")||inferTimeWindow(theme),themeCategory:themeNames[0]||"",methods:split(r["拍摄方式"]||"待规划"),media:split(r["素材类型"]||"待规划"),clarity:String(r["通透度要求"]||"低"),status:(statuses.includes(String(r["拍摄状态"]) as Status)?String(r["拍摄状态"]):"未拍摄") as Status,note:String(r["备注"]||""),sourceRow:i+2,longitude,latitude,coordinateSystem,scheduleDate:r["计划日期"] instanceof Date?(r["计划日期"] as Date).toISOString().slice(0,10):String(r["计划日期"]||""),scheduleSlot:String(r["计划时段"]||""),stations:station?[{id:`s-${i}`,name:station,description:String(r["机位说明"]||"")}]:[],samples:String(r["样片链接"]||"").trim()?[{id:`p-${i}`,name:"Excel 样片",url:String(r["样片链接"])}]:[],retakeReason:String(r["补拍原因"]||""),missingShots:String(r["缺失镜头"]||""),graduationCriteria:String(r["毕业标准"]||"")};
    }).filter(t=>t.location);
    if(imported.length&&confirm(`识别到 ${imported.length} 条任务，覆盖 ${new Set(imported.map(item=>item.countryCode)).size} 个国家。替换当前数据吗？`)){
      const migrated=migrateWorkspace(imported);
      const importedPoints=migrated.points.map(point=>{const names=[...(creativeThemes.get(locationRecordKey(point))||[])];return names.length?{...point,themeNames:names}:point});
      setPoints(importedPoints);setTasks(migrated.tasks);setCityRecords(importedCities.length?mergeCityRecords(importedCities,importedPoints,migrated.tasks):deriveCityRecords(importedPoints,migrated.tasks));setCountry("ALL");setCity("ALL");setDistrict("全部区县");setView("library");
    }
  }
  async function exportExcel(){
    const XLSX=await import("xlsx");
    const rows=taskViews.map(t=>{const point=points.find(item=>item.id===t.pointId);const geography=normalizeGeography(point||t);return{国家代码:geography.countryCode,国家:geography.countryName,"省/州":geography.admin1,城市:geography.city,区县:geography.district,时区:geography.timezone,完整地址:geography.formattedAddress||"",点位名称:t.location,点位优先级:point?.priority||t.priority,拍摄任务:t.theme,拍摄时间:t.timeWindow||"自定义",创作主题:(point?.themeNames||[]).join("、"),拍摄方式:t.methods.join("、"),素材类型:t.media.join("、"),通透度要求:t.clarity,拍摄状态:t.status,计划日期:t.scheduleDate||"",计划时段:t.scheduleSlot||"",关联机位:(t.stationIds||[]).map(id=>point?.stations.find(station=>station.id===id)?.name).filter(Boolean).join("、"),全部机位:(point?.stations||[]).map(station=>station.name).join("、"),经度:point?.longitude||"",纬度:point?.latitude||"",坐标系:point?.coordinateSystem||"",补拍原因:t.retakeReason||"",缺失镜头:t.missingShots||"",毕业标准:t.graduationCriteria||"",样片链接:(t.samples||[]).map(s=>s.url.startsWith("data:")?"本地样片":s.url).join("、"),备注:t.note}});
    const cityRows=cityRecords.map(record=>({国家代码:record.countryCode,国家:record.countryName,"省/州":record.admin1,城市:record.city,时区:record.timezone,点亮状态:record.status,到访日期:record.visitedAt||"",城市中心经度:record.longitude||"",城市中心纬度:record.latitude||"",关联点位数:points.filter(point=>cityKey(point)===cityKey(record)).length,城市备注:record.note}));
    const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(rows),"全球摄影点位数据");XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(cityRows),"城市足迹");XLSX.writeFile(wb,`摄影点位_${new Date().toISOString().slice(0,10)}.xlsx`)
  }
  ...
  async function createPoint(){if(!(await ensureAdmin()))return;const id=crypto.randomUUID();const selectedCity=points.find(point=>(country==="ALL"||point.countryCode===country)&&(city==="ALL"||cityKey(point)===city))||cityRecords.find(record=>(country==="ALL"||record.countryCode===country)&&(city==="ALL"||cityKey(record)===city));const geography=normalizeGeography(selectedCity?cityGeography(selectedCity as CityRecord):DEFAULT_GEOGRAPHY);if(district!=="全部区县")geography.district=district;const point:PointRecord={...geography,id,location:`新拍摄点位 ${points.length+1}`,priority:"低",themeNames:[],stations:[]};setPoints(items=>[point,...items]);setExpanded(id);setPointEditOnOpen(true);setView("library")}
  async function savePointGroup(groupItem:ReturnType<typeof group>[number],patch:Geography&{location:string;longitude?:number;latitude?:number;coordinateSystem?:"wgs84"|"gcj02"},themeNames:string[],stations:Station[],taskWindows:string[]){if(!(await ensureAdmin()))return false;const pointId=groupItem.point.id;const validStationIds=new Set(stations.map(station=>station.id));const geography=normalizeGeography(patch);setPoints(items=>items.map(point=>point.id===pointId?{...point,...patch,...geography,themeNames,stations,coordinateSystem:patch.coordinateSystem||(patch.longitude&&patch.latitude?"gcj02":point.coordinateSystem)}:point));setTasks(items=>{const retained=items.map(task=>task.pointId===pointId?{...task,...geography,location:patch.location,stations,stationIds:(task.stationIds||[]).filter(id=>validStationIds.has(id))}:task);const existing=new Set(retained.filter(task=>task.pointId===pointId).map(task=>task.timeWindow||inferTimeWindow(task.theme)));let nextId=Math.max(0,...retained.map(task=>task.id));const additions:Task[]=taskWindows.filter(window=>!existing.has(window)).map(window=>({...geography,id:++nextId,pointId,location:patch.location,priority:"低",theme:window,timeWindow:window,themeCategory:"",methods:["待规划"],media:["照片"],clarity:"中",status:"未拍摄",note:"",sourceRow:0,stationIds:[],stations,samples:[]}));return [...additions,...retained]});setPointEditOnOpen(false);setExpanded(null);return true}
  ...
  async function addTask(groupItem:ReturnType<typeof group>[number]){if(!(await ensureAdmin()))return;const id=Math.max(0,...tasks.map(task=>task.id))+1;const point=groupItem.point;const task:Task={...normalizeGeography(point),id,pointId:point.id,location:point.location,priority:"低",theme:"日出",timeWindow:"日出",themeCategory:"",methods:["待规划"],media:["照片"],clarity:"中",status:"未拍摄",note:"",sourceRow:0,stationIds:[],stations:point.stations,samples:[]};setTasks(items=>[task,...items]);setExpanded(null);setEditing(id)}
  ...
  async function saveCityRecord(record:CityRecord,previous?:CityRecord){if(!(await ensureAdmin()))return false;const normalized=normalizeCityRecord(record);const nextKey=cityKey(normalized);if(cityRecords.some(cityRecord=>cityRecord.id!==previous?.id&&cityKey(cityRecord)===nextKey)){alert("该城市已经存在，请直接编辑现有城市。");return false}if(previous){const previousKey=cityKey(previous);const geography={countryCode:normalized.countryCode,countryName:normalized.countryName,admin1:normalized.admin1,city:normalized.city,timezone:normalized.timezone};setCityRecords(records=>records.map(cityRecord=>cityRecord.id===previous.id?{...normalized,id:previous.id}:cityRecord));setPoints(items=>items.map(point=>cityKey(point)===previousKey?{...point,...geography}:point));setTasks(items=>items.map(task=>cityKey(task)===previousKey?{...task,...geography}:task))}else setCityRecords(records=>[...records,normalized].sort((a,b)=>`${a.countryName}${a.city}`.localeCompare(`${b.countryName}${b.city}`,"zh-CN")));return true}
  async function deleteCityRecord(record:CityRecord){if(!(await ensureAdmin()))return false;const key=cityKey(record);const pointIds=new Set(points.filter(point=>cityKey(point)===key).map(point=>point.id));setCityRecords(records=>records.filter(cityRecord=>cityRecord.id!==record.id));setPoints(items=>items.filter(point=>!pointIds.has(point.id)));setTasks(items=>items.filter(task=>!task.pointId||!pointIds.has(task.pointId)));if(city===key){setCountry("ALL");setCity("ALL");setDistrict("全部区县")}return true}
  function openCityPoints(record:CityRecord){setCountry(record.countryCode);setCity(cityKey(record));setDistrict("全部区县");setStatus("全部状态");setPriority("全部优先级");setCategory("全部归类");setQuery("");setView("library")}
  ...
  const nav=[{id:"library",label:"点位库"},{id:"gallery",label:"样片画廊"},{id:"footprint",label:"足迹地图"},{id:"map",label:"地图天气"},{id:"calendar",label:"拍摄日历"},{id:"themes",label:"创作主题"},{id:"coverage",label:"覆盖分析"}] as const;
...
{view==="library"&&<div className="homeOverview"><AstronomyHero/><OverviewStats pointCount={groups.length} cityCount={new Set(points.map(cityKey)).size} counts={counts} scheduleCount={calendarEvents.length} coordinateCount={points.filter(point=>point.longitude&&point.latitude).length}/></div>}
{view!=="gallery"&&<><section className="intro">
<div>
<p className="eyebrow">PHOTO LOCATION ATLAS · WORKSPACE</p>
<h1>{view==="library"?"把想去的地方，拍得更完整。":view==="footprint"?"把走过的城市，连成自己的摄影地图。":view==="map"?"跨越城市，先看天再出发。":view==="calendar"?"把好天气留给重要机位。":view==="themes"?"按主题，整理每一个机位。":"每一个空白，都有下一次出发。"}</h1>
<p>共 {groups.length} 个点位、{tasks.length} 条拍摄任务；点位修改保存在当前设备，云端样片长期保存。</p>
</div>
{view==="library"?<div className="libraryIntroActions"><CurrentWeatherCard/><Button size="lg" onClick={createPoint}><PlusIcon data-icon="inline-start"/>新建点位</Button></div>:view!=="calendar"&&view!=="themes"&&view!=="footprint"&&<Button size="lg" onClick={createPoint}><PlusIcon data-icon="inline-start"/>新建点位</Button>}
</section>
</>}

  {view==="library"&&<section className="workspace">
<aside className="districtSidebar" aria-label="全球地点筛选">
<div className="asideTitle">
<span>地点范围</span>
<small>{countries.length} 国 · {cities.length} 城</small>
</div>
<div className="geoFilterStack">
<Field><FieldLabel>国家或地区</FieldLabel><Select value={country} onValueChange={value=>{setCountry(value||"ALL");setCity("ALL");setDistrict("全部区县")}}><SelectTrigger className="geoFilterTrigger"><SelectValue placeholder="全部国家"/></SelectTrigger><SelectContent><SelectGroup><SelectItem value="ALL">全部国家</SelectItem>{countries.map(item=><SelectItem key={item.code} value={item.code}>{item.name}</SelectItem>)}</SelectGroup></SelectContent></Select></Field>
<Field><FieldLabel>城市</FieldLabel><Select value={city} onValueChange={value=>{setCity(value||"ALL");setDistrict("全部区县")}}><SelectTrigger className="geoFilterTrigger"><SelectValue placeholder="全部城市"/></SelectTrigger><SelectContent><SelectGroup><SelectItem value="ALL">全部城市</SelectItem>{cities.map(item=><SelectItem key={item.key} value={item.key}>{item.name}{item.admin1&&item.admin1!==item.name?` · ${item.admin1}`:""}</SelectItem>)}</SelectGroup></SelectContent></Select></Field>
</div>{["全部区县",...districts].map(d=>
<button key={d} className={district===d?"district active":"district"} onClick={()=>setDistrict(d)}>
<span>{d}</span>
<b>{d==="全部区县"?groups.filter(g=>(country==="ALL"||g.point.countryCode===country)&&(city==="ALL"||cityKey(g.point)===city)).length:groups.filter(g=>(country==="ALL"||g.point.countryCode===country)&&(city==="ALL"||cityKey(g.point)===city)&&g.district===d).length}</b>
...
<div className="spotList">{filtered.slice(0,pointDisplayLimit).map(g=>{
  ...
  <div className="pointCardHeader"><span className={`priorityBadge priority-${g.priority}`}>{g.priority}优先</span><span className="pointDistrict">{g.point.city} · {g.district}</span><span className="pointCompletion">完成 {doneTasks.length}/{g.tasks.length}</span></div>
...
{filtered.length>pointDisplayLimit&&<div className="progressiveLoad"><span>为保证浏览流畅，点位按批次加载</span><Button variant="outline" onClick={()=>setPointPage({key:pointFilterKey,limit:pointDisplayLimit+48})}>再加载 {Math.min(48,filtered.length-pointDisplayLimit)} 个点位</Button></div>}
  ...
  {view==="library"&&activeGroup&&<PointDetailModal
    key={`${activeGroup.key}-${pointEditOnOpen?"edit":"view"}`}
    point={activeGroup}
    initialEditing={pointEditOnOpen}
    categories={themeCategories}
    onClose={()=>{setExpanded(null);setPointEditOnOpen(false)}}
    onSave={(patch,names,stations,taskWindows)=>savePointGroup(activeGroup,patch,names,stations,taskWindows)}
    onAddTask={()=>addTask(activeGroup)}
    onManageTask={async id=>{if(await openEditor(id))setExpanded(null)}}
  ...
  {view==="footprint"&&<FootprintAtlas cities={cityRecords} points={points} tasks={taskViews} ensureAdmin={ensureAdmin} onSaveCity={saveCityRecord} onDeleteCity={deleteCityRecord} onOpenPoints={openCityPoints}/>}
...
<small>{amapLocating?"正在通过高德定位中国境内点位…":"中国境内使用高德路线 · 海外路线跳转 Google Maps"}</small>
</div>
</div>
...
function PointDetailModal({point,initialEditing,categories,onClose,onSave,onAddTask,onManageTask,onRemoveTask,onChangeStatus}:{point:ReturnType<typeof group>[number];initialEditing?:boolean;categories:string[];onClose:()=>void;onSave:(patch:Geography&{location:string;longitude?:number;latitude?:number;coordinateSystem?:"wgs84"|"gcj02"},themeNames:string[],stations:Station[],taskWindows:string[])=>Promise<boolean>;onAddTask:()=>void;onManageTask:(id:number)=>void;onRemoveTask:(task:Task)=>void;onChangeStatus:(task:Task)=>void}){
  const source=point.point;const themeOptions=[...new Set([...categories,...source.themeNames])];
  const existingTaskWindows=new Set(point.tasks.map(task=>task.timeWindow||inferTimeWindow(task.theme)));const [editingPoint,setEditingPoint]=useState(Boolean(initialEditing));const [saving,setSaving]=useState(false);const [formError,setFormError]=useState("");const [selectedThemes,setSelectedThemes]=useState<string[]>(source.themeNames);const [selectedTaskWindows,setSelectedTaskWindows]=useState<string[]>(shootTimes.filter(window=>existingTaskWindows.has(window)));const [draftStations,setDraftStations]=useState<Station[]>(source.stations);const [draft,setDraft]=useState({...normalizeGeography(source),location:point.location,longitude:source.longitude,latitude:source.latitude,coordinateSystem:source.coordinateSystem});
  useEffect(()=>{const previous=document.body.style.overflow;document.body.style.overflow="hidden";return()=>{document.body.style.overflow=previous}},[]);
  async function save(){if(!draft.location.trim()){setFormError("请填写点位名称");return}if(!draft.longitude||!draft.latitude){setFormError("请在地图中搜索并选择拍摄位置");return}setSaving(true);setFormError("");try{if(await onSave({...normalizeGeography(draft),location:draft.location.trim(),longitude:draft.longitude,latitude:draft.latitude,coordinateSystem:draft.coordinateSystem},selectedThemes,draftStations.filter(station=>station.name.trim()).map(station=>({...station,name:station.name.trim()})),selectedTaskWindows))onClose()}finally{setSaving(false)}}
...
<div className="modalHead"><div><small>{editingPoint?"EDIT LOCATION":"LOCATION DETAIL"}</small><h2 id="point-dialog-title">{editingPoint?"新建或编辑拍摄点位":point.location}</h2><p>{editingPoint?"搜索全球地点确定位置，国家、城市、区县和时区将自动关联。":`${geographyLabel(source,"district")} · ${point.tasks.length} 个拍摄任务 · ${point.stations.length} 个机位`}</p></div>{!editingPoint&&<button onClick={onClose} aria-label="关闭点位详情">×</button>}</div>
{editingPoint?<div className="pointEditGrid">
<FieldGroup className="pointBaseFields"><Field><FieldLabel htmlFor="point-name">点位名称</FieldLabel><Input id="point-name" value={draft.location} onChange={event=>{setDraft({...draft,location:event.target.value});setFormError("")}}/></Field><Field><FieldLabel>地点层级</FieldLabel><div className="pointGeoPreview"><Badge variant="outline">{draft.countryName||"待识别国家"}</Badge><Badge variant="outline">{draft.city||draft.admin1||"待识别城市"}</Badge><Badge variant="outline">{draft.district||"待识别区县"}</Badge><Badge variant="secondary">{draft.timezone||"UTC"}</Badge></div></Field></FieldGroup>
<div className="pointMapSection"><strong>地图位置</strong><p>搜索城市、建筑、道路或附近地标；选择结果后可直接点击地图微调，无需输入经纬度。</p><PointMapPicker longitude={draft.longitude} latitude={draft.latitude} geography={normalizeGeography(draft)} onPick={value=>{setDraft(current=>({...current,...normalizeGeography({...current,...value}),longitude:value.longitude,latitude:value.latitude,coordinateSystem:value.coordinateSystem||current.coordinateSystem}));setFormError("")}}/></div>
...
function Gallery({tasks,points,categories,ensureAdmin,onRemoteSamplesChange,onEdit}:{tasks:Task[];points:PointRecord[];categories:string[];ensureAdmin:()=>Promise<boolean>;onRemoteSamplesChange:(samples:GallerySample[])=>void;onEdit:(id:number)=>void}){
  ...
  const [query,setQuery]=useState(""); const [galleryCountry,setGalleryCountry]=useState("ALL"); const [galleryCity,setGalleryCity]=useState("ALL"); const [district,setDistrict]=useState("全部区县"); const [theme,setTheme]=useState("全部主题"); const [category,setCategory]=useState("全部归类");
  ...
  const local=useMemo(()=>tasks.flatMap(t=>{const station=t.stations?.find(item=>(t.stationIds||[]).includes(item.id))||t.stations?.[0];const point=points.find(item=>item.id===t.pointId);return (t.samples||[]).map(s=>({...normalizeGeography(point||t),id:`local-${t.id}-${s.id}`,url:s.url,uploadedAt:"",taskId:String(t.id),district:t.district,location:t.location,theme:t.theme,themeCategory:point?.themeNames[0]||"",stationId:station?.id||"",stationName:station?.name||"未指定机位",stationDescription:station?.description||"",subjectDescription:"",note:t.note||"",originalName:s.name,local:true} as GallerySample))}),[tasks,points]);
  const all=[...remote,...local].map(normalizeGallerySample);const sampleCountries=[...new Map(all.map(item=>{const geo=normalizeGeography(item);return[geo.countryCode,{code:geo.countryCode,name:geo.countryName}]})).values()];const sampleCities=[...new Map(all.filter(item=>galleryCountry==="ALL"||normalizeGeography(item).countryCode===galleryCountry).map(item=>{const geo=normalizeGeography(item);return[cityKey(geo),{key:cityKey(geo),name:geo.city||geo.admin1}]})).values()];const districts=[...new Set(all.filter(item=>(galleryCountry==="ALL"||normalizeGeography(item).countryCode===galleryCountry)&&(galleryCity==="ALL"||cityKey(normalizeGeography(item))===galleryCity)).map(x=>x.district).filter(Boolean))]; const themes=[...new Set(all.map(x=>x.theme).filter(Boolean))];
  const shown=all.filter(x=>{const geo=normalizeGeography(x);return(galleryCountry==="ALL"||geo.countryCode===galleryCountry)&&(galleryCity==="ALL"||cityKey(geo)===galleryCity)&&(district==="全部区县"||x.district===district)&&(theme==="全部主题"||x.theme===theme)&&(category==="全部归类"||(x.themeCategory||"")===category)&&`${x.location} ${geographyLabel(geo,"district")} ${x.stationName} ${x.device||""} ${x.shootTime||""} ${x.themeCategory||"未归类"} ${x.theme} ${x.subjectDescription||""} ${x.note} ${x.originalName}`.toLowerCase().includes(query.toLowerCase())});
  ...
  const galleryFilterKey=`${galleryCountry}\u0000${galleryCity}\u0000${district}\u0000${theme}\u0000${category}\u0000${query}`;const groupDisplayLimit=galleryPage.key===galleryFilterKey?galleryPage.limit:60;
...
<Select value={galleryCountry} onValueChange={value=>{setGalleryCountry(value||"ALL");setGalleryCity("ALL");setDistrict("全部区县")}}><SelectTrigger className="galleryGeoSelect"><SelectValue placeholder="全部国家"/></SelectTrigger><SelectContent><SelectGroup><SelectItem value="ALL">全部国家</SelectItem>{sampleCountries.map(item=><SelectItem key={item.code} value={item.code}>{item.name}</SelectItem>)}</SelectGroup></SelectContent></Select>
<Select value={galleryCity} onValueChange={value=>{setGalleryCity(value||"ALL");setDistrict("全部区县")}}><SelectTrigger className="galleryGeoSelect"><SelectValue placeholder="全部城市"/></SelectTrigger><SelectContent><SelectGroup><SelectItem value="ALL">全部城市</SelectItem>{sampleCities.map(item=><SelectItem key={item.key} value={item.key}>{item.name}</SelectItem>)}</SelectGroup></SelectContent></Select>
<select value={district} onChange={e=>setDistrict(e.target.value)}>
<option>全部区县</option>{districts.map(x=>
<option key={x}>{x}</option>)}</select>
<select value={category} onChange={e=>setCategory(e.target.value)}>
...
<option key={t.id} value={t.id}>{t.city||t.admin1} · {t.district} · {t.location} · {t.timeWindow||"自定义"} · {t.theme}</option>)}</select>
</label>
...
<button className="primary" disabled={uploading||!stationName.trim()} onClick={upload}>{uploading?progress:"开始上传"}</button>
  ...
  <Dialog open={Boolean(active)} onOpenChange={open=>{if(!open)closeEditor()}}>{active&&<DialogContent className="sampleEditorDialog" showCloseButton={!savingMeta}>
    ...
    <aside className="sampleEditorPanel"><div className="sampleEditorHeading"><div><p className="eyebrow">REFERENCE EDITOR</p><h2>编辑样片</h2><p>图片处理和资料修改会在点击保存后一次提交。</p></div><Badge variant="secondary">{normalizeGeography(active).city} · {active.district}</Badge></div>
...
function Coverage({points,tasks,categories,samples}:{points:PointRecord[];tasks:Task[];categories:string[];samples:GallerySample[]}){
  ...
  const countryStats=[...new Set(points.map(point=>point.countryCode))].map(code=>{const related=points.filter(point=>point.countryCode===code);const done=related.filter(point=>pointState(point)==="已毕业").length;return{name:related[0]?.countryName||code,total:related.length,done,open:related.length-done}}).sort((a,b)=>b.total-a.total||a.name.localeCompare(b.name,"zh-CN"));
  const cityStats=[...new Set(points.map(cityKey))].map(key=>{const related=points.filter(point=>cityKey(point)===key);const done=related.filter(point=>pointState(point)==="已毕业").length;return{name:`${related[0]?.city||related[0]?.admin1||"待分类城市"} · ${related[0]?.countryName||""}`,total:related.length,done,open:related.length-done}}).sort((a,b)=>b.total-a.total||a.name.localeCompare(b.name,"zh-CN"));
  const districtCount=new Set(points.map(point=>`${cityKey(point)}::${point.district}`)).size;
  ...
  const locationGap=[...cityStats].sort((a,b)=>b.open-a.open||b.total-a.total)[0];const themeGap=[...categoryStats].filter(item=>item.total>0).sort((a,b)=>a.percent-b.percent||b.open-a.open)[0];const windowGap=[...windowStats].sort((a,b)=>(b.unshot+b.redo)-(a.unshot+a.redo))[0];
  ...
  return <section className="coverage coverageDashboard">
    <div className="coverageIntro"><div><p className="eyebrow">COVERAGE INTELLIGENCE</p><h2>摄影资产覆盖分析</h2><p>同时观察点位毕业、任务状态、国家城市、创作主题、拍摄时段与资料完整度；点位下全部任务完成后才计为毕业。</p></div><Badge variant="outline">{points.length} 个点位 · {tasks.length} 条任务 · {samples.length} 张云端样片</Badge></div>
    <div className="coverageTopGrid">
      ...
      <div className="coverageMetricGrid">
        <CoverageMetric icon={MapIcon} label="全球地点触达" value={`${countryStats.length} 国 · ${cityStats.length} 城`} detail={`${districtCount} 个区县已有摄影点位`}/>
    ...
    <div className="coverageChartGrid">
      <Card><CardHeader><CardTitle>城市完成分布</CardTitle><CardDescription>按城市点位总量排序，比较已毕业与未毕业点位</CardDescription><CardAction><MapPinIcon/></CardAction></CardHeader><CardContent><ChartContainer config={coverageProgressConfig} className="coverageHorizontalChart"><BarChart accessibilityLayer data={cityStats.slice(0,12)} layout="vertical" margin={{left:8,right:12}}><CartesianGrid horizontal={false}/><YAxis dataKey="name" type="category" tickLine={false} axisLine={false} width={92}/><XAxis type="number" allowDecimals={false} tickLine={false} axisLine={false}/><ChartTooltip content={<ChartTooltipContent indicator="line"/>}/><ChartLegend content={<ChartLegendContent/>}/><Bar dataKey="done" stackId="city" fill="var(--color-done)" radius={[4,0,0,4]}/><Bar dataKey="open" stackId="city" fill="var(--color-open)" radius={[0,4,4,0]}/></BarChart></ChartContainer></CardContent></Card>
    ...
    <Card className="coverageInsightCard"><CardHeader><CardTitle>系统归纳</CardTitle><CardDescription>根据当前数据自动提取最值得优先处理的覆盖缺口</CardDescription></CardHeader><CardContent className="coverageInsightGrid"><article><span>城市缺口</span><strong>{locationGap?.name||"暂无数据"}</strong><p>{locationGap?`还有 ${locationGap.open} 个点位未毕业，共 ${locationGap.total} 个点位。`:"添加点位后将自动分析。"}</p></article><article><span>主题短板</span><strong>{themeGap?.name||"暂无关联主题"}</strong><p>{themeGap?`毕业度 ${themeGap.percent}%，还有 ${themeGap.open} 个关联点位未完成。`:"先为点位关联创作主题。"}</p></article><article><span>时段缺口</span><strong>{windowGap?.name||"暂无任务"}</strong><p>{windowGap?`未完成 ${windowGap.unshot+windowGap.redo} 条，其中 ${windowGap.redo} 条待补拍。`:"创建拍摄任务后将自动分析。"}</p></article><article><span>资料短板</span><strong>{weakestReadiness?.label||"暂无数据"}</strong><p>{weakestReadiness?`当前 ${weakestReadiness.value} / ${weakestReadiness.total}，完整度 ${completionPercent(weakestReadiness.value,weakestReadiness.total)}%。`:"暂无需要补齐的资料。"}</p></article></CardContent></Card>
...
<h2>{task.location}</h2>
<p>{task.city||task.admin1} · {task.district} · 拍摄任务 · {task.timeWindow||"自定义时段"}</p>
</div>
