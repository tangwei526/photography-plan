...
const themeIcon=(name:string)=>{const icons:Record<string,string>={彩虹:"rainbow.png",朝霞:"dawn.png",晚霞:"sunset.png",寺庙:"temple.png",日月对齐:"sun-moon.png",太阳月亮同框:"sun-moon.png",桥梁:"bridge.png",大桥:"bridge.png",立交:"bridge.png",雷电:"lightning.png",雨天:"rain.png",轨道交通:"transit.png",地铁:"transit.png"};return icons[name]?`${assetBase}theme-icons/${icons[name]}`:""};
...
<div className="themeGrid">{records.map(record=>{const items=tasks.filter(task=>task.themeCategory===record.name);const stationCount=items.reduce((sum,task)=>sum+(task.stations?.length||0),0);const done=items.filter(task=>task.status==="已毕业").length;const icon=themeIcon(record.name);return <article className="themeCard" key={record.id}>
<button className="themeCardOpen" onClick={()=>setActiveId(record.id)}><span className="themeCardIcon">{icon?<img src={icon} alt=""/>:<b>{record.name.slice(0,1)}</b>}</span><span className="themeCardCopy"><small>{stationCount} 个机位</small><strong>{record.name}</strong><em>{items.length} 个拍摄任务 · {done} 个已毕业</em><i><span style={{width:`${done/(items.length||1)*100}%`}}/></i></span><span className="themeCardArrow">↗</span></button>
<div className="themeCardActions"><button onClick={()=>onRename(record)}>编辑名称</button><button onClick={()=>onDelete(record)}>删除主题</button></div>
</article>})}</div></section>
